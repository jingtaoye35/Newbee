"""FinancialReportCashflowService: 现金流量表 full_init + daily_update.
...
from datasource.adapter.finance import fetch_financial_report_cashflow
...
def _fetch_one(code: str) -> pd.DataFrame:
    return fetch_financial_report_cashflow(code)


class FinancialReportCashflowService:
    """现金流量表服务."""

    def __init__(self, *, root: Optional[str] = None) -> None:
        self.root = Path(root) if root else None
        self.dtype = FinancialReportCashflow
    ...
    def full_init(
        ...
        *,
        ...
        logger.info(f"[FinancialReportCashflow] full_init: {len(codes)} stocks")
        summary, all_dfs = parallel_chunked_fetch(
            "Financial_Report_Cashflow",
            codes,
            _fetch_one,
            desc="[FinancialReportCashflow] full_init",
        )
        if not all_dfs:
            return {"rows": int(self.file_.stats().row_count), "skipped": True}

        result = upsert_and_update_state(
            self.file_, self.state, "Financial_Report_Cashflow", all_dfs=all_dfs
        )
    ...
    def daily_update(self, *, today: Optional[str] = None, source: str = "em") -> dict[str, int]:
        ...
        start, end = self.state.resume_range("Financial_Report_Cashflow", latest=today_str)
        if start > end:
            return {"rows": int(self.file_.stats().row_count), "skipped": True}
        return self.full_init(start=start, source=source)


__all__ = ["FinancialReportCashflowService"]
