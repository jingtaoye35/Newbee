...
读取 `datas/Stock_KData.parquet` (long format), 按 (trade_date, stock_code) pivot
出 (T, N) 矩阵, 返回 `Bars` dataclass. 供 `alpha_backend.engines` 和 `scripts/`
使用, 替代旧的 `alpha_backend.datas.storage.load_bars_from_parquet`.

设计要点:
  - stock_ids 始终是 9 字符 stock_code (与 Universe.parquet 对齐),
    不再使用 6 位 stock_id
  - close / adj_close: 历史 schema 有 `close_adj` (后复权收盘价), M2 已移除.
    当前 `adj_close` 字段等价于 `close` (与 vendor 拉到的 close 口径一致:
    sina 前复权, em/tx/baostock 后复权). 保留字段名仅为后端代码兼容性.
  - `matrix` 属性: (T, N, 6) 堆叠 [open, high, low, close, volume, amount],
...
class Bars:
    ...
    Attributes:
        ...
        adj_close: 等价于 close (向后兼容字段). 复权口径由 vendor 拉取时决定
                   (sina 前复权, em/tx/baostock 后复权). 业务需要严格复权时
                   按 Stock_Basic_Data.adj_factor 自行 join.
    """
    ...
    def active_mask_at(self, asof: str) -> np.ndarray:
        """当期活跃股票 mask (N,).
...
def load_bars(
    ...
    *,
    ...
    """从 `datas/Stock_KData.parquet` 读全市场 K 线, 拼成 ndarray(T, N).

    Args:
        stock_codes: 要加载的股票列表 (9 字符 .SH/.SZ), 顺序即最终 N 列顺序
        start / end: ISO date string "YYYY-MM-DD" (闭区间)
        root: datas 根目录, 默认 `alpha_backend/datasource/storage/bars_adapter.py` 之上 3 级 (PROJECT_ROOT)

    Returns:
        Bars (dates 升序; 缺数据的 cell 为 NaN)

    Raises:
        FileNotFoundError: Stock_KData.parquet 不存在
    """
    ...
    # trade_date 已是 ISO string, 按 ISO 升序即是交易日顺序
    trade_dates_asc = sorted(df["trade_date"].unique())
    dates = list(trade_dates_asc)
