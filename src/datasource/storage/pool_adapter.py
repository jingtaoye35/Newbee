"""`StockPool` adapter — 兼容 `alpha_backend.datas.universe.StockPool` 的 thin shim.

读取 `datas/Universe.parquet` (3 列: stock_index, stock_code, ipo_date),
...
from utils.tools import format_iso_date

# ---------- 默认路径 ----------

DEFAULT_POOL_PATH = Path("datas/Universe.parquet")
...
class StockPool:
    ...
    def _save(self) -> None:
        ...
        StockPool.load() 接收的是完整文件路径 (e.g. ``datas/Universe.parquet``),
        与 DataFile 的 root/storage_path 拆分约定不同. DataFile 新签名支持
        storage_path_override, 直接传文件名即可, 不再需要 dc_replace 绕路.
        """
        file_ = DataFile(Universe, root=self.path.parent, storage_path_override=self.path.name)
        file_.truncate()
    ...
    def active_count(self, asof: Optional[str] = None) -> int:
        """当期活跃数量 (默认今天)."""
        mask = self.active_mask(asof)
        return int(mask.sum())

    def active_mask(self, asof: Optional[str] = None) -> np.ndarray:
        ...
        if asof is None:
            asof_str = format_iso_date(date.today())
        else:
            asof_str = asof
    ...
    def stock_ids(self) -> list[str]:
        ...
        if self._df.empty:
            return []
        return self._df.sort_values("stock_index")["stock_code"].tolist()
