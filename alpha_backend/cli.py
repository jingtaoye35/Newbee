"""alpha_backend 顶层 CLI.

用法:
  alpha_backend --help
  alpha_backend backtest <config.yaml>              # 组合回测
  alpha_backend alpha   <config.yaml>               # Alpha 回测 (IC, decile)
  alpha_backend datas status                         # 查看数据覆盖范围
  alpha_backend datas update [--dry-run]             # 增量拉取数据
"""
...
from alpha_backend.utils.config import (
...
def cmd_alpha(args: argparse.Namespace) -> int:
    ...
    from alpha_backend import alpha_store
    from alpha_backend.datasource.storage.bars_adapter import load_bars
    from alpha_backend.datasource.storage.pool_adapter import StockPool
...
def cmd_data_status(args: argparse.Namespace) -> int:
    """`alpha_backend datas status` — 转发到 alpha_backend.datasource.cli status."""
...
def cmd_data_update(args: argparse.Namespace) -> int:
    """`alpha_backend datas update` — 转发到 alpha_backend.datasource.cli update."""
    from alpha_backend.datasource import cli as ds_cli
...
def _build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        prog="alpha_backend",
        description="alpha_backend 量化交易平台 CLI (M1 骨架)",
    )
    parser.add_argument("--version", action="version", version="alpha_backend 0.1.0")
    ...
    def add_common(sp: argparse.ArgumentParser) -> None:
        ...
        sp.add_argument("--datas-root", type=Path, default=DEFAULT_DATA_ROOT,
    ...
    # datas (status / update)
    p_data = sub.add_parser("datas", help="数据层管理 (status / update)",
    ...
    p_ds.add_argument("--datas-root", type=Path, default=DEFAULT_DATA_ROOT,
                      help=f"datas 根目录 (默认 {DEFAULT_DATA_ROOT})")
    ...
    p_du.add_argument("--datas-root", type=Path, default=DEFAULT_DATA_ROOT,
                      help=f"datas 根目录 (默认 {DEFAULT_DATA_ROOT})")
...
def main(argv: list[str] | None = None) -> int:
    ...
    # datas 子命令未指定 status/update → 打 datas help
    if getattr(args, "command", None) == "datas" and not getattr(args, "data_command", None):
        sub_action = next(
        ...
        print("\nERROR: alpha_backend datas 需要 status / update 子命令.", file=sys.stderr)
        return 2
