"""test_trade_status_service.py — TradeStatusService + AdjFactorService 推断测试."""
...
def test_trade_status_handles_nan_ohlcv(tmp_path: Path) -> None:
    ...
    df = pd.DataFrame(
        [{"trading_date": "2024-01-02", "stock_code": "600000.SH",
          ...
          "amount": None, "volume": None, "close_post_adj": None}]
    )
    for col in ("open", "high", "low", "close", "amount", "volume", "close_post_adj"):
        df[col] = df[col].astype("float32")
...
def test_adj_factor_infer_from_kdata(tmp_path: Path) -> None:
    """adj_factor = close_post_adj / close."""
    _write_universe(tmp_path, ["600000.SH"])
    _write_kdata(
        tmp_path,
        [
            {"trading_date": "2024-01-02", "stock_code": "600000.SH",
             ...
             "volume": 1e7, "close_post_adj": 20.0},  # 后复权 = 2x
        ],
    )
    svc = AdjFactorService(root=str(tmp_path))
    result = svc.full_init(start="2024-01-01")
    assert result["rows"] == 1

    df = pd.read_parquet(tmp_path / "data" / "Adj_Factor.parquet")
    assert abs(float(df.iloc[0]["adj_factor"]) - 2.0) < 1e-6


def test_adj_factor_skips_zero_close(tmp_path: Path) -> None:
    """close == 0 → 跳过该行."""
    _write_universe(tmp_path, ["600000.SH"])
    _write_kdata(
        tmp_path,
    ...
    result = AdjFactorService(root=str(tmp_path)).full_init(start="2024-01-01")
    assert result["rows"] == 0
    # 文件不存在 (因为没数据写入)
    assert not (tmp_path / "data" / "Adj_Factor.parquet").exists()


def test_adj_factor_requires_kdata(tmp_path: Path) -> None:
    with pytest.raises(RuntimeError):
        AdjFactorService(root=str(tmp_path)).full_init(start="2024-01-01")